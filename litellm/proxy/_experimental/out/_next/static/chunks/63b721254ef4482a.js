...
client = openai.OpenAI(
	api_key="${v||"YOUR_LITELLM_API_KEY"}",
	base_url="${A}"
)`;switch(_){case n.CHAT:{let e=Object.keys(E).length>0,a="";if(e){let e=JSON.stringify({metadata:E},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();a=`,
...
`;break}case n.RESPONSES:{let e=Object.keys(E).length>0,a="";if(e){let e=JSON.stringify({metadata:E},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();a=`,
    extra_body=${e}`}let i=S.length>0?S:[{role:"user",content:y}];t=`
import base64
